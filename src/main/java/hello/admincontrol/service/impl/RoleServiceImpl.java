...
import hello.admincontrol.repository.PermEntryRepository;
import hello.admincontrol.repository.RoleRepository;
...
public class RoleServiceImpl implements RoleService {
	...
	public void updateRoleName(String oldRoleName, String newRoleName) {
        Role role = this.getRoleByRoleName(oldRoleName)
            .orElseThrow(() -> new NotFound("角色不存在"));
	...
	@Override
	public void addPermEntry(String parentDescriptor, PermEntryItemDTO permx) {
        this.cachetree = null;
        ...
        MyBeanUtils.myCopyProperties(perm, permx);
	...
	@Override
	public void updatePermEntry(String descriptor, PermEntryItemDTO perm) {
        this.cachetree = null;
        perm.setDescriptor(descriptor);
        final PermEntry entry = this.getPermEntry(descriptor);
        MyBeanUtils.myCopyProperties(entry, perm);
    ...
    private PermEntryTreeDTO PermEntryToPermEntryTree(PermEntry entry)
    {
        PermEntryTreeDTO ans = new PermEntryTreeDTO();
        ArrayList<PermEntryTreeDTO> children = new ArrayList<>();
        MyBeanUtils.myCopyProperties(ans, entry);
	...
	public Collection<PermEntryTreeDTO> getPermEntryTree()
    {
        ...
        final Collection<PermEntryTreeDTO> ans = new ArrayList<>();
    ...
    private Collection<PermEntryTreeDTO> cachetree;

	@Override
	public Collection<PermEntryItemDTO> getPermEntries(String roleName)
    {
        Collection<PermEntryItemDTO> ans = new ArrayList<>();
	...
	@Override
	public PermEntryItemDTO getPermEntryItemByDescriptor(String descriptor)
    {
        PermEntryItemDTO item = new PermEntryItemDTO();
        MyBeanUtils.myCopyProperties(item, this.getPermEntry(descriptor));
        return item;
	}

	@Override
	public boolean hasPermission(String roleName, String descriptor) {
        return this.permEntryRepository
                   .findByDescriptorAndRoles_roleName(descriptor, roleName)
                   .isPresent();
	}
